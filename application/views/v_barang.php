...
<table class="table table-hover table-stripped"> 
	...
	<thead>
		...
		<tr>
			...
			<td>no</td><td>kode_barang</td><td>image</td><td>nama_barang</td><td>kategori</td><td>harga</td><td>stok</td><td></td><td></td>
	...
	<tbody>
		...
		<?php $no = 0; foreach($barang as $bg): $no++;?>
		...
		<tr>
			...
			<td><?=$no?></td><td><?=$bg->kode_barang?></td><td><img src="<?=base_url('assets/gambar/'.$bg->image)?>" style="width:40px;"></td><td><?=$bg->nama_barang?></td><td><?=$bg->nama_kategori?></td><td><?=$bg->harga?></td><td><?=$bg->stok?></td>

			<td><?php if($this->session->userdata('level')=="admin"){?> <a href="#ubah" data-toggle="modal" onclick="edit(<?=$bg->kode_barang?>)"  class="btn btn-warning">Ubah</a><?php }else{ 		echo "anda kasir"; }?></td>

			<td><?php if($this->session->userdata('level')=="admin"){?><a href="<?=base_url('index.php/barang/hapus/'.$bg->kode_barang)?>" onclick="return confirm('apakah anda yakin untuk menghapus')" class="btn btn-danger">Hapus</a><?php }else{ echo "anda kasir"; }?></td>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<form action="<?=base_url('index.php/barang/tambah')?>" method="post" enctype="multipart/form-data">
					...
					<tr>
							<td>foto image</td>
							<td><input type="file" name="image" style="margin-left: 20px;"></td>
						</tr>
						<tr>
							<td>nama barang</td>
			...
			<div class="modal-body">
				...
				<form action="<?=base_url('index.php/barang/update')?>" method="post" enctype="multipart/form-data">
					...
					<table>
						...
						<tr>
							<td>foto image</td>
							<td><input type="file" name="image"   id="image" style="margin-left: 20px;"></td>
						</tr>
